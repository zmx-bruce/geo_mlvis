...
point <- st_point(centers[1,], dim = "XY") %>%
    st_buffer(0.1) %>%
    st_geometry()
x_raster <- read_subset(vrt_path, st_bbox(point))
x <- as.array(x_raster)
...
np$save(file.path(params$out_dir, str_c("x-", params$B,"-",j, ".npy")), patch$x)
np$save(file.path(params$out_dir, str_c("y-", params$B,"-",j, ".npy")), patch_y)
write_sf(patch$meta, file.path(params$out_dir, str_c("geo-", params$B,"-",j, ".geojson")))
```
Write all results.
