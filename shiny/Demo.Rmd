---
title: "Shiny App for prediction results"
author: "Bruce Zheng"
date: "3/30/2021"
...
```{r}
path<-getwd()
iconurl<-paste(path,"/9521617427822_.pic.jpg",sep="")
geojson_path<-paste(path,"/glaciers.geojson",sep="")
basins_path<-paste(path,"/test_list-small.csv",sep="")
preds_path<-paste(path,"/preds",sep="")
...
```{r}
y_path <- file.path(geojson_path)
basins <- read_csv(basins_path)
```
...
paths<-prediction_paths(preds_path)
```
...
server<-shinyServer(function(input, output, session){
    ...
    output$grid<-renderPlot({
        id<-ifelse(is.null(click()$id),1,click()$id)
        result<-paths[paths$ix==id& paths$split=="train",]
        y <- {np$load(result[result$type=="y",]$path)}
        y_hat<-np$load(result[result$type=="y_hat",]$path)
